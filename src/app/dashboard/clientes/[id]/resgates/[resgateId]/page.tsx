// app/dashboard/clientes/[id]/resgate/[resgateId]/page.tsx
'use client';
...
// Interface para o resgate montado
interface ResgateDetalhado {
  id: string;
  quantidade: number;
  resgatadoEm: string;
  cupomId: string;
  cupom: CupomResumo & { loja?: { id: string; nome: string; logo?: string } };
  cliente: {
    id: string;
    nome: string;
    email: string;
...
export default function DetalhesResgatePage() {
  ...
  const resgateId = params?.resgateId as string;
  ...
  const carregarDados = useCallback(async () => {
    try {
      ...
      console.log('🔍 Buscando QR codes do resgate (admin):', { clienteId: id, resgateId });

      // Buscar QR codes do resgate diretamente via endpoint específico
      const qrCodesData = await clienteService.getQrCodesPorResgate(id, resgateId);
      
      console.log('📦 QR Codes recebidos:', qrCodesData);

      if (!qrCodesData || qrCodesData.length === 0) {
        toast.error('Nenhum QR code encontrado para este resgate');
      ...
      const resgateData: ResgateDetalhado = {
        ...
        resgatadoEm: primeiroQr.resgate?.resgatadoEm ?? primeiroQr.usadoEm,
        cupomId: primeiroQr.cupomId,
        cupom: {
          ...primeiroQr.cupom,
          loja: primeiroQr.cupom.loja,
        },
        cliente: {
          id: clienteData.id,
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <Link 
                href={`/dashboard/clientes/${resgate.cliente.id}`}
              ...
              <TableBody>
                {resgate.qrCodes.map((qrCode) => (
                  <TableRow key={qrCode.id} className="hover:bg-gray-50">
                    ...
                    <TableCell>
                      <Link href={`/dashboard/clientes/${resgate.cliente.id}/qrcode/${qrCode.id}`}>
